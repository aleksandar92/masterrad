...
namespace RDFSParserOWL2.Manager
{
    ...
    class StringManipulationManager
    {
        ...
        public static string ExtractShortestName(string fullName, string separator)
        {
            ...
            {
                int index = extarcted.LastIndexOf(separator, StringComparison.OrdinalIgnoreCase);
                if (index > -1)
                {
        ...
        public static bool IsValidRelativeURI(string uri)
        {
            string pattern = @"^http://[0-9a-zA-Z_-]*[.]{1}[0-9a-zA-Z._-]*/[0-9a-zA-Z_-]*[#]$";
            Regex reg = new Regex(pattern);
            if (reg.IsMatch(uri))
            {
                return true;
        ...
        public static string ExtractAllAfterSeparator(string fullName, string separator)
        {
            ...
            if ((extarcted != null) && (extarcted.Length > 0))
            {
                int index = extarcted.IndexOf(separator, StringComparison.OrdinalIgnoreCase);
        ...
        public static string ExtractAllBeforeSeparator(string fullName, string separator)
        {
            string extarcted = fullName;
            if ((extarcted != null) && (extarcted.Length > 0))
            {
                ...
                if ((index > -1) && (extarcted.Length > (index + 1)))
                {
                    extarcted = extarcted.Substring(0, index - 1);
                }
            }
            return extarcted;
        }
        ...
        /// <summary>
        /// Method extracts everithing after last occurence of given separator string in given fullName.
        /// </summary>
        /// <param fullName="fullName">full fullName</param>
        /// <param fullName="separator">separator character or string used in full fullName</param>
        /// <returns>shortest fullName ie. substring after last separator in full fullName</returns>
        public static string ExtractAllWithSeparator(string fullName, string separator)
        {
            string extarcted = fullName;
            if ((extarcted != null) && (extarcted.Length > 0))
            {
                int index = extarcted.LastIndexOf(separator, StringComparison.OrdinalIgnoreCase);
                if ((index > -1) && (extarcted.Length > (index)))
                {
                    extarcted = extarcted.Substring(index);
                }
            }
            return extarcted;
        }
        ...
        public static string TrimAfterStringIncludinString(string fileName, string subStr)
        {
            int minIndex = Int32.MaxValue;

            int index = fileName.IndexOf(subStr);
            ...
            {
                fileName = fileName.Substring(0, minIndex);
            }
